...
class CNNRealNumber(object):
    def __init__(self, sent_length, class_num,
        ...
        with tf.name_scope("linear"):
            ...
            self.linear_result_1 = tf.nn.xw_plus_b(self.h_drop, weights, bias, name="linear")
        ...
        with tf.name_scope("output"):
            ...
            l2_loss += tf.nn.l2_loss(weights)
            l2_loss += tf.nn.l2_loss(bias)
            self.linear_result_2 = tf.nn.xw_plus_b(self.softmax, weights, bias, name="linear2")
            self.output = 5 * tf.exp(self.linear_result_2) / (1 + tf.exp(self.linear_result_2)) - 2.5
            self.predictions = tf.to_int64(self.output + 2.5)

        with tf.name_scope("loss"):
            losses = tf.reduce_mean(tf.square(self.output + 2 - tf.to_float(tf.argmax(self.input_y, 1))))
